...
export function renderStreaks(state) {
  ...
  // Pre-calculate streaks to avoid O(N log N) redundant calls to an expensive function
  const routinesWithStreaks = state.routines.map(rt => ({
    routine: rt,
    streakInfo: calculateStreak(rt, state.completions, todayStr)
  }));

  // Sort routines by priority / streak
  const sortedRoutines = routinesWithStreaks.sort((a, b) => {
    return b.streakInfo.currentStreak - a.streakInfo.currentStreak;
  });

  sortedRoutines.forEach(item => {
    const rt = item.routine;
    const { currentStreak, maxStreak } = item.streakInfo;
    const color = CATEGORY_COLORS[rt.category] || '#00f0ff';
